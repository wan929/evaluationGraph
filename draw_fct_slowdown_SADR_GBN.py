...
parser.add_argument("-p", type = int , default = 2) # all, avg, 50, 95, 99, 999, max
...
linewide = 1.5
methods = ['GBN', 'LinkGuardianNB', 'Libra-S', 'Libra-C']
# -------------------------------
# Origin SADR_SV SADR_CV SADR_SV_NF SADR_CV_NF LinkGuardian LinkGuardian_NB SADR_CV_SLR
file = "/home/wan/Experimental_Graph/paper/3_Facebook/fh_20ms_1000_dcqcn_SADR_GBN.csv"
...
NB_avg = [float(x) for x in LGNB_avg]
NB_50 = [float(x) for x in LGNB_50]
NB_95 = [float(x) for x in LGNB_95]
NB_99 = [float(x) for x in LGNB_99]
NB_999 = [float(x) for x in LGNB_999]
NB_max = [float(x) for x in LGNB_max]
# -------------------------------
# log(2)处理
...
NB_avg = np.log(NB_avg)/np.log(2)
NB_50 = np.log(NB_50)/np.log(2)
NB_95 = np.log(NB_95)/np.log(2)
NB_99 = np.log(NB_99)/np.log(2)
NB_999 = np.log(NB_999)/np.log(2)
NB_max = np.log(NB_max)/np.log(2)
# -------------------------------
PER = ""
if args.w == "Facebook":
...
# -------------------------------
# red, dimgray, magenta, limegreen, orange, dodgerblue，green
ax1.plot(x, o_avg, color = 'red', linestyle = '-', label = 'GBN', marker = 'x', linewidth = linewide)
ax1.plot(x, NB_avg, color = 'green', linestyle = '-', label = 'LinkGuardianNB', marker = 'o', linewidth = linewide)
ax1.plot(x, SV_avg, color = 'orange', linestyle = '-', label = 'Libra-S', marker = 'v', linewidth = linewide)
ax1.plot(x, CV_avg, color = 'dodgerblue', linestyle = '-', label = 'Libra-C', marker = '^', linewidth = linewide)


ax2.plot(x, o_99, color = 'red', linestyle = '-', label = 'GBN', marker = 'x', linewidth = linewide)
ax2.plot(x, NB_99, color = 'green', linestyle = '-', label = 'LinkGuardianNB', marker = 'o', linewidth = linewide)
ax2.plot(x, SV_99, color = 'orange', linestyle = '-', label = 'Libra-S', marker = 'v', linewidth = linewide)
ax2.plot(x, CV_99, color = 'dodgerblue', linestyle = '-', label = 'Libra-V', marker = '^', linewidth = linewide)

# -------------------------------

fig.legend(methods, loc='upper center', bbox_to_anchor=(0.5, 1.12), fancybox=True, ncol=4, prop = font3, frameon = False)#, columnspacing = 1.15
# plt.subplots_adjust(wspace=0.27) #facebook
plt.subplots_adjust(wspace=0.27) #websearch
# plt.subplots_adjust(wspace=0.25) #google
...
if args.w =="Facebook":
    ...
    if args.lossrate == 1000:
        ...
        ax2.set_ylim(2, 5)
        ax2.set_yticks(np.arange(2, 6.1, 1), (4, '', 16, '', 64), fontproperties = font)
elif args.w == "Google":
    ax1.set_xlim(0, 9)
    ...
    if args.lossrate == 1000:
        ax1.set_ylim(0, 2)
        ax1.set_yticks(np.arange(0, 2.1, 1), (1, 2, 4), fontproperties = font)
        ax2.set_ylim(1, 5)
        ax2.set_yticks(np.arange(1, 5.1, 1), (2, 4, 8, 16, 32), fontproperties = font)
elif args.w == "WebSearch":
    ax1.set_xlim(0, 10)
    ...
    if args.lossrate == 1000:
        ...
        ax2.set_ylim(2, 9)
        ax2.set_yticks(np.arange(2, 9.1, 1), (4, '',16,'',64,'',256, ''), fontproperties = font)
# -------------------------------
if args.w == 'WebSearch':
    picture_name = '%s_80ms_%d_%s_Libra_GBN_NB_avg_99'%(args.w, args.lossrate, args.cc)
    ...
    picture_name = '%s_20ms_%d_%s_Libra_GBN_NB_avg_99'%(args.w, args.lossrate, args.cc)
    plt.savefig('/home/wan/Experimental_Graph/paper/3_Facebook/%s.pdf'%picture_name, format = 'pdf' , dpi = 800 ,bbox_inches = 'tight')
elif args.w == 'Google':
    picture_name = '%s_30ms_%d_%s_Libra_GBN_NB_avg_99'%(args.w, args.lossrate, args.cc)
